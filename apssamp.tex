...
\author{Siyang Li$^{1}$, Avi Vajpeyi$^1$, Patricio Maturana Russel$^{1,2}$, Renate Meyer$^1$ and Guillaume Boileau$^3$}
\affiliation{$^1$ Department of
  Statistics, University of Auckland, Auckland 1142, New Zealand \\
$^2$ Engineering, Computer and Mathematical Sciences, \\Auckland University of Technology, New Zealand\\
...
%    f(x) = -\frac{1}{\pi} \lim_{R \to \infty} \int_{-\infty}^\infty \frac{\sin[R(x-\theta)]}{x-\theta}f(\theta)d\theta
%\end{equation}
%and
...
%    \hat{f}_\text{nFI}(x) = -\frac{1}{n\pi}\sum_{i = 1}^n \frac{\sin(R(x - \theta^{(i)}))}{x - \theta^{(i)}} = -\hat{f}_\text{FI}(x)
%\end{equation}
%and
...
\begin{definition}
    ...
    %\begin{equation*} 
     %   \hat{f}_\text{nFI}(x) = \bigg|\frac{1}{n\pi}\sum_{i = 1}^n \frac{\sin(R(x - \theta^{(i)}))}{x - \theta^{(i)}}\bigg|
    %\end{equation*}
    %and
